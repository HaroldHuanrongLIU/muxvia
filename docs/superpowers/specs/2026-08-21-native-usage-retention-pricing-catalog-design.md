...
7. Retention keeps the current local day plus the configured number of preceding detailed local dates. Only strictly older completed dates are rolled up and pruned. A late-imported old Native Usage Record is additively merged into an existing Daily Usage Rollup before pruning.
