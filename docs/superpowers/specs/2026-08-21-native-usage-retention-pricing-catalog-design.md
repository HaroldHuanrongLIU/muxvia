...
3. Cursor advancement, all records parsed from the newly completed lines, first-fill pricing, and enforcement of the persisted retention period commit in one SQLite transaction. A malformed or incomplete final line is not counted and is retried on the next scan.
