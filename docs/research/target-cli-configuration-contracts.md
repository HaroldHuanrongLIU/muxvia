...
Status: researched against first-party documentation and source through 2026-08-15.
...
**Documented.** Claude Code currently exposes five provider-mode selectors: `CLAUDE_CODE_USE_BEDROCK`, `CLAUDE_CODE_USE_VERTEX`, `CLAUDE_CODE_USE_FOUNDRY`, `CLAUDE_CODE_USE_MANTLE`, and `CLAUDE_CODE_USE_ANTHROPIC_AWS`. The normal boolean convention treats `1` or `true` as enabled, `0` or `false` as disabled, and an empty settings-file value as unset for provider selection. An embedding host can also set `CLAUDE_CODE_PROVIDER_MANAGED_BY_HOST`; in that mode Claude Code ignores provider-selection, endpoint, and authentication variables from settings files, including `ANTHROPIC_BASE_URL` and `ANTHROPIC_API_KEY`. [Environment-variable precedence and boolean values](https://code.claude.com/docs/en/env-vars), [host-managed provider behavior](https://code.claude.com/docs/en/env-vars#variables)

**Muxvia decision.** Own only the three takeover `env` keys shown above, recording each prior value or absence. A higher-scope value for any owned key blocks guaranteed takeover. Any active documented provider-mode selector, or observable host-managed provider mode, blocks takeover without clearing or taking ownership of that selector. Treat an unrecognized non-empty selector value as incompatible rather than guessing how an installed Claude Code version interprets it.
...
**Documented.** A Claude Code gateway using the Anthropic Messages format must expose `POST /v1/messages`; `POST /v1/messages/count_tokens` is optional because Claude Code falls back to a local context estimate when it is absent. Inference may be posted to `/v1/messages?beta=true`, so a gateway must route by path while preserving the query rather than matching the complete URI literally. The token-counting endpoint is the only optional model endpoint; `HEAD /` is a best-effort startup connectivity probe that a gateway may reject without breaking inference. [Claude Code gateway protocol](https://code.claude.com/docs/en/llm-gateway-protocol)

Claude Code requires `anthropic-version` and `anthropic-beta` to reach an Anthropic-format upstream unchanged. `anthropic-beta` is an open-ended comma-separated capability list and must not be reduced to an allowlist. The client also sends correlation metadata including `X-Claude-Code-Session-Id`, `X-Claude-Code-Agent-Id`, and `X-Claude-Code-Parent-Agent-Id`; a gateway may consume those fields for routing, attribution, or tracing. Future `anthropic-*` and `x-claude-code-*` headers and future body fields are part of an open-list contract. With gateway model discovery explicitly enabled, Claude Code requests `/v1/models` at startup; discovery is off by default and is not needed for Muxvia's provider editor. [Gateway request headers and open-list forwarding](https://code.claude.com/docs/en/llm-gateway-protocol#request-headers), [gateway model discovery](https://code.claude.com/docs/en/llm-gateway-protocol#model-discovery)

Messages requests include, among other fields, `model`, `messages`, `max_tokens`, system content, and tools. With `stream: true`, the response is SSE with message, content-block, delta, ping, error, and stop events. The gateway must stream events as they arrive rather than buffer a complete response; clients must tolerate future event types. Anthropic's versioning policy permits compatible additions, including new optional inputs, response values, and enum-like event types. [Messages API](https://platform.claude.com/docs/en/api/messages/create), [streaming Messages](https://platform.claude.com/docs/en/build-with-claude/streaming), [API versioning](https://platform.claude.com/docs/en/api/versioning)

Claude Code retries some feature rejections by matching the upstream error wording and may disable the rejected capability for the rest of the conversation. A gateway must therefore preserve upstream error status and body rather than wrap the error in its own envelope; after an SSE response has begun, an upstream failure is an `event: error` inside the HTTP 200 stream. [Gateway retry and error forwarding](https://code.claude.com/docs/en/llm-gateway-protocol#automatic-retries-and-error-forwarding), [Anthropic error shapes](https://platform.claude.com/docs/en/api/errors), [streaming error events](https://platform.claude.com/docs/en/build-with-claude/streaming#error-events)

**Muxvia decision.** Although token counting is optional in the upstream gateway contract, Muxvia v1 will expose it because the product acceptance contract requires exact token-counting traffic. Implement a forward-compatible Anthropic Messages ingress, validate the Muxvia bearer credential before any upstream call, consume rather than forward that Routing Credential, and preserve the query, version/beta headers, correlation metadata, upstream error payloads, SSE order, and unknown fields supported by the selected adapter. An Anthropic-to-Anthropic adapter should pass all body fields and `anthropic-*` headers through except for the exact model/routing fields it explicitly owns. Do not depend on Claude Code's unpublished prompts, tool schemas beyond the public wire payload, or terminal UI behavior.
...
3. **Claude provider-mode ownership.** Block the five currently documented provider selectors—Bedrock, Vertex, Foundry, Mantle, and Claude Platform on AWS—and observable `CLAUDE_CODE_PROVIDER_MANAGED_BY_HOST` state without owning or clearing those keys. Add new provider selectors to the compatibility matrix deliberately rather than matching every unrelated `CLAUDE_CODE_USE_*` variable.
